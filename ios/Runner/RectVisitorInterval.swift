...
class RectVisitorInterval {
    //: static let shared = AppPermissionTool()
    static let shared = RectVisitorInterval()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func windowSource(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func back(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func polaroidCamera(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
